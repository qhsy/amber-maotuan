...
public class MyCollectionsActivity extends BaseActivity implements RefreshLay.OnRefreshListener, OnReceiveOttoEventInterface, BaseQuickAdapter.RequestLoadMoreListener{
    ...
    List<ArtVideoInfo> collecteVideoList = new ArrayList<>();
...
//        homeAdapter.setLoadMoreView(new CustomLoadMoreView());
    ...
    public void request() {
        if (checkNet()) {
            RequestUtils.getCollectionList(getRequestUnicode(), mRequestParams, this);
        } else {
    ...
    public void onHttpRequestSuccess(String url, HttpContext httpContext) {
        ...
        if (result.status == 1) {
            if (mRequestParams.pageOption.pageNum == 0) {
                ...
                if (videoList) {
                    collecteVideoList.addAll(result.collecteVideoList);
                }
    ...
    private void addEmptyView() {
        ...
        imageView.setImageResource(R.drawable.nodata_collect);
